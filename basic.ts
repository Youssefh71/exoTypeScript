...
let aa: number[] = [];
aa.push(33);
aa.push("abc"); // 🚨 ERROR : Type 'string' is not assignable to type 'number'.
...
bb = [1, 2, 3]; // 🚨 ERROR : Type 'string' is not assignable to type 'number'.
...
cc = { // 🚨 ERROR :the variable must have two properties
houseNumber: 33,
};
